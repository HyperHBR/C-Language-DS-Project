...
void Change_Of_Players_Location(struct Node *arr[GAME_SIZE*GAME_SIZE],int Change)
{
    struct Node *tp = arr[Current_Location]; //Ano Locho Che,Jo arr[Current_Location] Lakhu To Work Kare Che But tp Nathi Kartu
    if(tp->data==1)
    {
        tp->data = 0;
    }
    ...
    printf("\nYou Are Here: %d",Current_Location+1);
    printf("\nData Here: %d",arr[Current_Location]->data);
    delay(2000);

    if(arr[Current_Location]->data == 11 && arr[Current_Location]->data != 0 && arr[Current_Location]->data != 1)
    {
        printf("\nLadder Encountered At %d",Current_Location+1);
        Change_Of_Players_Location(arr,arr[Current_Location]->jump_size);

    }
    else if(arr[Current_Location]->data == 55 && arr[Current_Location]->data != 0 && arr[Current_Location]->data != 1)
    {
